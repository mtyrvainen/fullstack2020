...
blogsRouter.post('/', async (req, res) => {
  ...
  const blogWithUser = await Blog.findById(savedBlog.id).populate('user', { username: 1, name: 1 } )
  res.json(blogWithUser.toJSON())
})

blogsRouter.post('/:id/comments', async (req, res) => {
  const body = req.body
  console.log('Tried to add comment', body)
  ...
  console.log('Posting comment: ', blog)
